...
def window_forward(model, df:pd.DataFrame, window_days:int=40, test_days:int=2)->pd.DataFrame:
    ...
    for i in tqdm(range(0, len(df), test_days)):        
        df_train = df.iloc[i:i+window_days-test_days]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fxed Sliding Window Model Training and Prediction")
...
if __name__ == "__main__":
    ...
    logging.info(f"Fixed Sliding Window")
    ...
    svr_model = SVR(C=50, gamma=0.1, kernel="rbf")
